...
*Above: This isn't a one-shot generation. It’s the result of an agentic negative feedback loop.*

The problem isn't that LLMs can't code; it's that they can't *see* what they're coding. To bridge this gap, I built an OpenSCAD skill for my Pi agent harness that creates a tight **negative feedback loop**:
...
*Above: Gemma's initial attempt (V1). Recognizable, but spatially confused.*

![Dramatic improvement in Gemma V2 after skill optimization](images/gemma-v2-success.png)
*Above: Gemma V2. By optimizing the skill rather than the model, the same open-weight 31B parameter model produced a vastly superior, high-fidelity result.*
...
*Above: The Wild Turkey model, showing the agent's ability to handle organic, "low-poly" aesthetics.*
